 ...
 */
public fun formatEnemyList(enemyList: List<Enemy>, title: String) : String{
    if (enemyList.isEmpty()){ //when empty avoid all normal steps
        val content = " * nothing * "
    ...
    enemyList.forEach(){ //find how long longest poi name is in POIList
    ...
    enemyList.forEachIndexed { index, element ->
        ...
        if ((index+1) % POIsPerRow == 0 && index != enemyList.size-1) { //if index is last of row, but not last index
            toReturn+= "\n"; //new line
        }
    }
